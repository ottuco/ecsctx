...
- Masking has two entry points, both driving the same `MaskPIIFilter`: `mask_sensitive_data` (structlog processor, in the formatter's own chain) and the filter, attached only to handlers whose formatter does not run `mask_sensitive_data` (mail, third-party, plain console) — each record is masked once. Never scanned (`DEFAULT_SKIP_KEYS`/`DEFAULT_SKIP_LEAVES`, `masking/filters.py`): `service`/`project`/`log`, correlation ids (`session_id`, `trace`, `span`, `transaction.id`), `host`, `labels`, `user.name`, `http.request.method`, `http.response.status_code`, `url.domain`; extend with `ECSCTX_MASK_SKIP_PATHS`.
- Two independent detection strategies inside `MaskPIIFilter`: 17 ordered content rules (`masking/patterns.py::RULES` — order is load-bearing, see the comments above `_RULE_TABLE`), grouped into packs (`default` always on; `pci`, `financial_ids` opt-in via `masking/config.py`), each behind a literal pre-check; and key-name matching (`classify_key()`, whole words, cached). Any matching key — PII category or secret — blanket-masks its whole value regardless of type or nesting: a dict/list under a sensitive key is masked as one unit, never recursed into. Card keys truncate a PAN value (`mask_card_value`).
