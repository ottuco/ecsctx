...
PEM_CASES = [
    (
        "pem-private-key",
        pem("PRIVATE KEY", "MIIEvQIBADANBgkqhkiG9w0BAQEFAASC"),
        "[PEM-KEY-MASKED]",
    ),
    (
        "pem-rsa-private-key",
        pem("RSA PRIVATE KEY", "MIIEpAIBAAKCAQEA0abcDEF"),
        "[PEM-KEY-MASKED]",
    ),
    (
        "pem-ec-private-key",
        pem("EC PRIVATE KEY", "MHcCAQEEIABxYZec012private"),
        "[PEM-KEY-MASKED]",
    ),
    (
        "pem-public-key",
        pem("PUBLIC KEY", "MIIBIjANBgkqhkiG9w0pubKEY"),
        "[PEM-KEY-MASKED]",
    ),
    (
        "pem-rsa-public-key",
        pem("RSA PUBLIC KEY", "MEgCQQCrsaPUBLICkeyXYZ"),
        "[PEM-KEY-MASKED]",
    ),
]
...
CREDENTIAL_CASES = [
    ...
    (
        "cred-single-quoted-colon-tight",
        "'token':'abcd1234'",
        "'token':'[SECRET-MASKED]'",
    ),
    ("cred-double-quoted-colon", '"token": "abcd1234"', '"token": "[SECRET-MASKED]"'),
    (
        "cred-double-quoted-colon-tight",
        '"token":"abcd1234"',
        '"token":"[SECRET-MASKED]"',
    ),
    ...
    (
        "cred-short-token-equals-in-sentence",
        "message token=1",
        "message token=[SECRET-MASKED]",
    ),
    ("cred-token", "token abcd1234", "token [SECRET-MASKED]"),
    ("cred-anyword_token", "anyword_token abcd1234", "anyword_token [SECRET-MASKED]"),
    (
        "cred-any_word_token",
        "any_word_token abcd1234",
        "any_word_token [SECRET-MASKED]",
    ),
    (
        "cred-any-word_token",
        "any-word_token abcd1234",
        "any-word_token [SECRET-MASKED]",
    ),
    (
        "cred-any-word-token",
        "any-word-token abcd1234",
        "any-word-token [SECRET-MASKED]",
    ),
    (
        "cred-any_1-word_2-token",
        "any_1-word_2-token abcd1234",
        "any_1-word_2-token [SECRET-MASKED]",
    ),
    ("cred-secret", "secret abcd1234", "secret [SECRET-MASKED]"),
    (
        "cred-any_1-word_2-secret",
        "any_1-word_2-secret abcd1234",
        "any_1-word_2-secret [SECRET-MASKED]",
    ),
    ("cred-password", "password abcd1234", "password [SECRET-MASKED]"),
    (
        "cred-any_1-word_2-password",
        "any_1-word_2-password abcd1234",
        "any_1-word_2-password [SECRET-MASKED]",
    ),
    ("cred-passwd", "passwd abcd1234", "passwd [SECRET-MASKED]"),
    (
        "cred-any_1-word_2-passwd",
        "any_1-word_2-passwd abcd1234",
        "any_1-word_2-passwd [SECRET-MASKED]",
    ),
    ...
    (
        ...
        "authorization_header [SECRET-MASKED]",
    ),
    (
        "cred-authorization-header",
        "authorization-header abcd1234",
        "authorization-header [SECRET-MASKED]",
    ),
    ("cred-authorisation", "authorisation abcd1234", "authorisation [SECRET-MASKED]"),
    (
        "cred-authorisation_header",
        "authorisation_header abcd1234",
        "authorisation_header [SECRET-MASKED]",
    ),
    (
        "cred-authorisation-header",
        "authorisation-header abcd1234",
        "authorisation-header [SECRET-MASKED]",
    ),
    ...
    (
        ...
        "encryption-key [SECRET-MASKED]",
    ),
    (
        "cred-decryption-key",
        "decryption-key abcd1234",
        "decryption-key [SECRET-MASKED]",
    ),
    ...
    (
        "cred-numeric-secret-in-card-digit-range",
        "secret_key=1234567890123456",
        "secret_key=[SECRET-MASKED]",
    ),
    (
        "auth-header-line-single-quotes",
        f"'Authorization': 'Bearer {JWT}'",
        "'Authorization': '[SECRET-MASKED]'",
    ),
    (
        "auth-header-line-double-quotes",
        f'"Authorization": "Bearer {JWT}"',
        '"Authorization": "[SECRET-MASKED]"',
    ),
    (
        "auth-header-line-colon",
        f"Authorization: Bearer {JWT}",
        "Authorization: [SECRET-MASKED] [JWT-MASKED]",
    ),
    (
        "auth-header-line-equal",
        f"Authorization= Bearer {JWT}",
        "Authorization= [SECRET-MASKED] [JWT-MASKED]",
    ),
    (
        "auth-header-line-space",
        f"Authorization Bearer {JWT}",
        "Authorization Bearer [SECRET-MASKED]",
    ),
    (
        "auth-header-quoted-kv",
        f'{{"Authorization": "{HEX}"}}',
        '{"Authorization": "[SECRET-MASKED]"}',
    ),
    (
        "auth-header-quoted-kv-in-sentence",
        f'Here is {{"Authorization": "{HEX}"}}',
        'Here is {"Authorization": "[SECRET-MASKED]"}',
    ),
    (
        "auth-dict-value-with-spaces",
        f'{{"Authorization": "Bearer {HEX} more"}}',
        '{"Authorization": "[SECRET-MASKED]"}',
    ),
    (
        "authorization-raw-colon",
        f"Authorization: {HEX}abcd",
        "Authorization: [SECRET-MASKED]",
    ),
    (
        "authorisation-raw-dict",
        f'{{"Authorisation": "{HEX}abcd"}}',
        '{"Authorisation": "[SECRET-MASKED]"}',
    ),
    (
    ...
    (
        ...
        "refresh_token=[SECRET-MASKED]",
    ),
    (
        "client_secret-kv",
        "client_secret=sk_live_abcd1234ef",
...
CVV_CASES = [
    ...
    (
        "cvv-in-sentence",
        "Here cvv=100 is submitted",
        "Here cvv=[CVV-MASKED] is submitted",
    ),
...
PAYMENT_ID_CASES = [
    (
        "payment_id-bare-colon",
        "payment_id: abc12345",
        "payment_id: [PAYMENT-ID-MASKED]",
    ),
    (
        "payment_id-bare-equals",
        "payment_id= abc12345",
        "payment_id= [PAYMENT-ID-MASKED]",
    ),
    ("payment_id-bare-space", "payment_id abc12345", "payment_id [PAYMENT-ID-MASKED]"),
    (
        "payment-id-bare-colon",
        "payment-id: abc12345",
        "payment-id: [PAYMENT-ID-MASKED]",
    ),
    (
        "payment-id-bare-equals",
        "payment-id= abc12345",
        "payment-id= [PAYMENT-ID-MASKED]",
    ),
    ("payment-id-bare-space", "payment-id abc12345", "payment-id [PAYMENT-ID-MASKED]"),
    (
        "payment_id-single-quoted-colon",
        "'payment_id': 'abc12345'",
        "'payment_id': '[PAYMENT-ID-MASKED]'",
    ),
    (
        "payment-id-single-quoted-colon",
        "'payment-id': 'abc12345'",
        "'payment-id': '[PAYMENT-ID-MASKED]'",
    ),
    (
        "payment_id-single-quoted-colon-tight",
        "'payment_id':'abc12345'",
        "'payment_id':'[PAYMENT-ID-MASKED]'",
    ),
    (
        "payment-id-single-quoted-colon-tight",
        "'payment-id':'abc12345'",
        "'payment-id':'[PAYMENT-ID-MASKED]'",
    ),
    (
        "payment_id-double-quoted-colon",
        '"payment_id": "abc12345"',
        '"payment_id": "[PAYMENT-ID-MASKED]"',
    ),
    (
        "payment-id-double-quoted-colon",
        '"payment-id": "abc12345"',
        '"payment-id": "[PAYMENT-ID-MASKED]"',
    ),
    (
        "payment_id-double-quoted-colon-tight",
        '"payment_id":"abc12345"',
        '"payment_id":"[PAYMENT-ID-MASKED]"',
    ),
    (
        "payment-id-double-quoted-colon-tight",
        '"payment-id":"abc12345"',
        '"payment-id":"[PAYMENT-ID-MASKED]"',
    ),
    (
        "transaction-id-quoted-alnum-value",
        "'transaction_id': 'jbzzTT577'",
        "'transaction_id': '[PAYMENT-ID-MASKED]'",
    ),
    (
        "auth-id-quoted-numeric-value",
        "'auth_id': '016153570198200'",
        "'auth_id': '[PAYMENT-ID-MASKED]'",
    ),
    (
        "payment-id-numeric-in-card-digit-range",
        "payment_id: 1234567890123456 done",
        "payment_id: [PAYMENT-ID-MASKED] done",
    ),
    (
        "payment-id-equals-numeric-in-card-digit-range",
        "payment_id= 9876543210987654 done",
        "payment_id= [PAYMENT-ID-MASKED] done",
    ),
    (
        "transaction_id-bare-colon",
        "transaction_id: abcd1234",
        "transaction_id: [PAYMENT-ID-MASKED]",
    ),
    (
        "transaction-id-bare-colon",
        "transaction-id: abcd1234",
        "transaction-id: [PAYMENT-ID-MASKED]",
    ),
...
IBAN_CASES = [
    (
        "iban",
        "account GB33BUKB20201555555555 credited",
        "account [IBAN-MASKED] credited",
    ),
    (
        "iban-be-digit-run-collision",
        "acct BE68539007547034 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-fr-digit-run-collision",
        "acct FR1420041010050500013M02606 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-bh-digit-run-collision",
        "acct BH67BMAG00001299123456 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-qa-digit-run-collision",
        "acct QA58DOHB00001234567890ABCDEFG debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-digit-run-13-just-over-card-floor",
        "acct GB1212345678901 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-digit-run-16-classic-pan-length",
        "acct GB3412345678901234 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-digit-run-19-top-of-card-range",
        "acct GB5612345678901234567 debited",
        "acct [IBAN-MASKED] debited",
    ),
    (
        "iban-digit-run-20-past-card-range",
        "acct GB78123456789012345678 debited",
        "acct [IBAN-MASKED] debited",
    ),
]
...
PHONE_CASES = [
    ...
    (
        "phone-intl-country-code-in-card-digit-range",
        "call +44-555-123-4567 now",
        "call [PHONE-MASKED] now",
    ),
    (
        "phone-intl-3digit-country-code-in-card-digit-range",
        "call +971-555-123-4567 now",
        "call [PHONE-MASKED] now",
    ),
]
...
EMAIL_CASES = [
    ...
    (
        "email-single-quoted-colon",
        "'email': 'user@example.com'",
        "'email': '[EMAIL-MASKED]'",
    ),
    (
        "email-double-quoted-colon",
        '"email": "user@example.com"',
        '"email": "[EMAIL-MASKED]"',
    ),
    ...
    (
        "email-under-sensitive-parent-key",
        {"contact": {"email": "user@example.com"}},
        {"contact": "[GENERIC-MASKED]"},
    ),
]
...
JWT_CASES = [
    ...
    (
        "jwt-in-sentence-space-bounded",
        f"Here it's {JWT} failed!",
        "Here it's [JWT-MASKED] failed!",
    ),
    (
        "jwt-in-sentence-parens-wrapped",
        f"Here it's ({JWT}) failed!",
        "Here it's ([JWT-MASKED]) failed!",
    ),
    (
        "jwt-in-sentence-colon-prefixed",
        f"Here it's: {JWT} failed!",
        "Here it's: [JWT-MASKED] failed!",
    ),
    ...
    (
        "jwt-suffix-swallowed-in-sentence",
        f"token was {JWT}extra and more",
        "token was [JWT-MASKED] and more",
    ),
]
...
CARD_CASES = [
    ...
    (
        "card-19d-9-space-prefixed-in-sentence",
        "Here is 9123456789123456789 card Number",
        "Here is [CARD-MASKED] card Number",
    ),
    (
        "card-19d-9-paren-prefixed-in-sentence",
        "Here is (9123456789123456789) card Number",
        "Here is ([CARD-MASKED]) card Number",
    ),
    (
        "card-19d-9-bracket-prefixed-in-sentence",
        "Here is [9123456789123456789] card Number",
        "Here is [[CARD-MASKED]] card Number",
    ),
    (
        "card-19d-9-brace-prefixed-in-sentence",
        "Here is {9123456789123456789} card Number",
        "Here is {[CARD-MASKED]} card Number",
    ),
    (
        "card-19d-9-colon-prefixed-in-sentence",
        "Here num:9123456789123456789 card Number",
        "Here num:[CARD-MASKED] card Number",
    ),
    (
        "card-19d-9-equals-prefixed-in-sentence",
        "Here num=9123456789123456789 card Number",
        "Here num=[CARD-MASKED] card Number",
    ),
    (
        "card-19d-9-comma-prefixed-in-sentence",
        "Here num,9123456789123456789 card Number",
        "Here num,[CARD-MASKED] card Number",
    ),
    (
        "card-19d-9-dot-prefixed-in-sentence",
        "Here num.9123456789123456789 card Number",
        "Here num.[CARD-MASKED] card Number",
    ),
    ...
    (
        "card-17d-space-trailing-chunk-is-phone-shaped",
        "11234 56 7891234567",
        "[CARD-MASKED]",
    ),
...
SSN_CASES = [
    (
        "ssn-space-glued-no-separators",
        "ssn 123456789 on file",
        "ssn [SSN-MASKED] on file",
    ),
    ...
    (
        "ssn-space-separated-all-groups",
    ...
    (
        "ssn-dash-separated-all-groups",
        "ssn 123-45-6789 on file",
        "ssn [SSN-MASKED] on file",
    ),
    (
        "ssn-mixed-space-then-dash",
        "ssn 123 45-6789 on file",
        "ssn [SSN-MASKED] on file",
    ),
    (
        "ssn-mixed-dash-then-space",
        "ssn 123-45 6789 on file",
        "ssn [SSN-MASKED] on file",
    ),
    ...
    (
        "ssn-single-quote-prefixed",
        "ssn'123456789' on file",
        "ssn'[SSN-MASKED]' on file",
    ),
    (
        "ssn-double-quote-prefixed",
        'ssn"123456789" on file',
        'ssn"[SSN-MASKED]" on file',
    ),
...
DICT_KEY_CASES = [
    ...
    (
        "key-session-key-int-value",
        {"session_key": 12345},
        {"session_key": "[SECRET-MASKED]"},
    ),
    (
        "key-access-token-none-value",
        {"access_token": None},
        {"access_token": "[SECRET-MASKED]"},
    ),
...
NOT_MASKED_CASES = [
    ...
    (
        "cred-both-session-id-unaffected",
        "session_id=6ea04d6db060f7ce414b6f5faa7119161e2214bc",
    ),
...
ACCEPTED_LEAK_CASES = [
    ...
    (
        "card-19d-other-preceded-by-digit-space-unaffected",
        "point 1 1234567891234567891",
    ),
